...
def shiftHoods [w] [h] (offset: i32) (hoods: [w][h]hood) : [w][h]hood =
...
def divRoundingUp (x: i64) (y: i64) =
  (x + y - 1) / y

type game_state [w] [h] =
  { generation: i32 -- generation
...
def new_game_with (ww: i64, wh: i64) (e: element) : ext_game_state =
...
def screen_point_to_world_point ((ul_x, ul_y): (f32, f32))
                                (s: f32)
                                ((sw, sh): (i64, i64))
                                ((ww, wh): (i64, i64))
                                ((x, y): (i64, i64)) =
...
def elemColour (x: element) : argb.colour =
  if x == steam_water
...
def dist_sq (x0: f32, y0: f32) (x1: f32, y1: f32) : f32 =
  (x0 - x1) * (x0 - x1) + (y0 - y1) * (y0 - y1)

def f32p (x: i32, y: i32) : (f32, f32) =
  (r32 x, r32 y)
...
entry render ({generation = gen, hoods, width = ww, height = wh}: ext_game_state)
  ...
  let offset = gen % 2
  let particle_pixel (x: i64) (y: i64) = elemColour (worldIndex offset hoods (i32.i64 x, i32.i64 y))
...
def line_dist_sq (p: (f32, f32)) (v: (f32, f32)) (w: (f32, f32)) : f32 =
  let l2 = dist_sq v w
  in if l2 == 0f32
     then dist_sq p v
...
def line_dist (p: (i32, i32)) (v: (i32, i32)) (w: (i32, i32)) : f32 =
  f32.sqrt (line_dist_sq (f32p p) (f32p v) (f32p w))

entry add_element [h] [w]
                  ({ generation = gen
                   , hoods = hoods: [w][h]hood
                   , width = ww
                   , height = wh
                   }: ext_game_state
                  )
  ...
  let f x y =
    ...
    in hoodFromQuadrants (if line_dist ul_p from to < r32 r && ul == nothing then elem else ul)
...
entry clear_element [h] [w]
                    ({generation = gen, hoods = hoods: [w][h]hood, width = ww, height = wh}: ext_game_state)
                    (ul_x: f32)
                    (ul_y: f32)
                    (s: f32)
                    (sw: i64)
  ...
  let f x y =
    ...
    let dl_p = ((x * 2) + offset + 0, (y * 2) + offset + 1)
    let dr_p = ((x * 2) + offset + 1, (y * 2) + offset + 1)
    in hoodFromQuadrants (if line_dist ul_p from to < r32 r then nothing else ul)
