...
def hoodQuadrants ((ul, ur, dl, dr): hood) : (element, element, element, element) =
  (ul, ur, dl, dr)

def hoodFromQuadrants (ul: element) (ur: element) (dl: element) (dr: element) : hood =
  (ul, ur, dl, dr)

-- Return the requested quadrant from the given hood.
def hoodQuadrant (h: hood) (i: marg_pos) : element =
...
def indexToHood (offset: i32) (i: i32) : (i32, i32) =
  if offset == 0
  then (i / 2, i % 2)
  else ((i + 1) / 2, (i + 1) % 2)
...
def worldIndex [w] [h] (offset: i32) (elems: [w][h]hood) ((x, y): (i32, i32)) : element =
...
def hoodRandoms w h ((lower, upper): (i32, i32)) (gen: i32) : [w][h]i32 =
...
def checkIfDrop (above: element) (below: element) : (element, element) =
  if isWall above || isWall below || weight below >= weight above
  then (above, below)
  else (below, above)

-- Apply gravity within a hood.
def gravity (h: hood) : hood =
  let (ul, ur, dl, dr) = hoodQuadrants h
  let (ul, ur, dl, dr) =
...
-- array of hoods.
def one_step [w] [h] (gen: i32) (hoods: [w][h]hood) : [w][h]hood =
  let randomish = hoodRandoms w h (0, 10000) gen
  let envs =
    map2 (\randomish_r hoods_r -> map2 alchemy randomish_r hoods_r)
         randomish
         hoods
  in map2 (\r0 r1 -> map2 ageHood r0 r1)
          randomish
